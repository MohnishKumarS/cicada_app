                        ...
                        <tr class="cart-item" id="{{ $cartItem['product_id'] }}-{{ $cartItem['size'] }}">
                            <td class="cart-item__media">
                                <div class="cart-item__pic">
                                    <img src="{{ asset('admin-files/products/' . $product->main_img) }}" alt="{{ $product->product_name }}" class="cart-item__img img-fluid" width="100" height="100" loading="lazy">
                                </div>
                            </td>
                            <td class="cart-item__info">
                                <div>
                                    <div class="cart-item__name">
                                        <a href="{{ route('product.show', $product->slug) }}" class="link link--hover">{{ $product->product_name }}</a>
                                    </div>
                                    <div class="cart-item__price">Rs. {{ number_format($product->offer_price, 2) }}</div>
                                        ...
                                        <div class="product__quantity">
                                            <div class="input-group input-group-lg" style="width: 140px;">
                                                <span class="input-group-text" 
                                                      ...
                                                      data-size="{{ $cartItem['size'] }}">-</span>
                                                <input type="text" class="form-control text-center cart-quantity" value="{{ $totalQuantity }}" readonly>
                                                <span class="input-group-text" 
                                                      onclick="CartQuantityChange(this, 1)" 
                                                      data-product-id="{{$cartItem['product_id'] }}" 
                                                      data-size="{{ $cartItem['size'] }}">+</span>
                                            ...
                                            {{-- <div class="input-group input-group-lg" style="width: 140px;">
                                                <span class="input-group-text" onclick="CartQuantityChange(this, -1)">-</span>
                                                <input type="text" class="form-control text-center cart-quantity" value="{{ $totalQuantity }}" readonly>
                                                <span class="input-group-text" onclick="CartQuantityChange(this, 1)">+</span>
                                            </div> --}}
                                        </div>
                                        <div class="product__remove">
                                            <button onclick="removeItemFromCart('{{ $cartItem['product_id'] }}', '{{ $cartItem['size'] }}')">

                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" aria-hidden="true" focusable="false" class="icon icon-remove">
                                                    <path d="M14 3h-3.53a3.07 3.07 0 00-.6-1.65C9.44.82 8.8.5 8 .5s-1.44.32-1.87.85A3.06 3.06 0 005.53 3H2a.5.5 0 000 1h1.25v10c0 .28.22.5.5.5h8.5a.5.5 0 00.5-.5V4H14a.5.5 0 000-1z" fill="currentColor"></path>
                                                    <path d="M6.91 1.98c.23-.29.58-.48 1.09-.48s.85.19 1.09.48c.2.24.3.6.36 1.02h-2.9c.05-.42.17-.78.36-1.02zm4.84 11.52h-7.5V4h7.5v9.5z" fill="currentColor"></path>
                            ...
                            <td class="cart-item__totals lg-hide text-end">
                                <div class="cart-item__tprice">Rs. {{ number_format($product->offer_price * $totalQuantity) }}</div>
                            </td>
